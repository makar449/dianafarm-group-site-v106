...
/* v289 production page builder. Server is the source of truth; local storage is admin draft cache only. */
(function(){
  'use strict';

  const CONTENT_KEY = 'dfg_universal_edits_v289_admin_cache';
  const DESIGN_KEY = 'dfg_universal_design_v289_admin_cache';
  ...
  function saveContent(store){ writeJSON(CONTENT_KEY, store); persistUniversalEdits(); }
  function designStore(){ return readJSON(DESIGN_KEY, {}); }
  function saveDesign(store){ writeJSON(DESIGN_KEY, store); applyDesign(store); persistUniversalEdits(); }
  function universalEditsPayload(){
    return {
      version: 'v289-admin-editor',
  ...
  function persistUniversalEdits(){
    if(!window.DFG_BACKEND || !window.DFG_BACKEND.isConfigured || !window.DFG_BACKEND.isConfigured() || !window.DFG_BACKEND.saveUniversalEdits) return;
    window.clearTimeout(persistTimer);
    persistTimer = window.setTimeout(()=>{
      window.DFG_BACKEND.saveUniversalEdits(universalEditsPayload()).catch((error)=>{
        console.warn('Universal editor production save failed', error);
        toast('Правка сохранена локально, но production-сервер не принял изменения');
      });
    }, 350);
  }

  async function loadUniversalEditsFromServer(){
    if(remoteLoaded) return;
    remoteLoaded = true;
    if(!window.DFG_BACKEND || !window.DFG_BACKEND.isConfigured || !window.DFG_BACKEND.isConfigured() || !window.DFG_BACKEND.loadAdminUniversalEdits) return;
    try{
      const payload = await window.DFG_BACKEND.loadAdminUniversalEdits();
      if(payload && typeof payload === 'object'){
        if(payload.content && typeof payload.content === 'object') writeJSON(CONTENT_KEY, payload.content);
        if(payload.design && typeof payload.design === 'object') writeJSON(DESIGN_KEY, payload.design);
        applyDesign(designStore());
      }
    }catch(error){
      console.warn('Universal editor production load failed', error);
      toast('Production-правки не загрузились. Проверь авторизацию/сервер.');
    }
  }
  ...
  async function uploadEditableMedia(file){
    if(!window.DFG_BACKEND || !window.DFG_BACKEND.isConfigured || !window.DFG_BACKEND.isConfigured() || !window.DFG_BACKEND.uploadMedia){
      toast('Production-сервер недоступен: фото не сохранено. Запусти сайт через npm run start:production.');
      throw new Error('Production media upload is required. Base64 fallback is disabled in v289.');
    }
    try{
      const media = await window.DFG_BACKEND.uploadMedia(file);
      if(media && media.path) return media.path;
      throw new Error('Server did not return media path');
    }catch(error){
      console.warn('Production media upload failed', error);
      toast('Фото не загружено на сервер. Изменение отменено: проверь размер/тип файла и авторизацию.');
      throw error;
    }
  }
  ...
  async function saveElement(key){
    ...
    for(const fileInput of qsa('[data-v280-file]', form)){
      ...
      let value;
      try{ value = await uploadEditableMedia(fileInput.files[0]); }catch(error){ return; }
  ...
  function bind(){
    ...
    document.addEventListener('input', async (event)=>{
      ...
      if(previewFile && previewFile.files && previewFile.files[0]){
        ...
        if(img) img.src = await uploadEditableMedia(previewFile.files[0]);
      }
    });
  }
  ...
  async function boot(){
    injectNav();
    bind();
    await loadUniversalEditsFromServer();
    applyDesign();
  }
  ...
  window.DFGFullSiteBuilderV280 = { open:openBuilder, exportEdits, importEdits, clearEdits, persist:persistUniversalEdits, reload:loadUniversalEditsFromServer };
})();
