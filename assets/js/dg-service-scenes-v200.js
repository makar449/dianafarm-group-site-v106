...
(function(){
  'use strict';
  var VERSION = 'v200-document-bound-service-scenes';
  var SCENE_BY_PAGE = {
  ...
  var state = window.DG_PRO_SCENES_V200 = {version:VERSION, mounted:false, frames:0, scene:null, kind:null, errors:[], canvas:false, rootExists:false, visible:false};
  window.DG_PRO_SCENES_V196 = state; window.DG_PRO_SCENES_V198 = state;
  ...
  function computeStageBox(){
    ...
    var size = w >= 1680 ? 360 : (w <= 980 ? 280 : 320);
    ...
    if(layout && layout.hero){
      ...
      top = clamp(Math.round(hero.top + hero.height * 0.19), 120, Math.round(h * 0.34));
      if(copy && proof){
        var gapLeft = copy.right + 26;
        var gapRight = proof.left - 26;
        var gapWidth = gapRight - gapLeft;
        if(gapWidth > 210){
          size = Math.min(size, Math.max(250, gapWidth * 0.78));
