(function () {
  ...
  async function loadRemoteSiteData(options = {}) {
    ...
    try {
      ...
      if (signature === remoteDataSignature) return;
  ...
  function applyStaticPageCopy() {
    ...
    const copy = {
      ...
      ka: {
        services: ['საერთაშორისო მომსახურება სრული сопровождения формატით', 'სრული გადაწყვეტილებები ბიზნესისა და ცხოვრებისათვის ევროპაში, აზიასა და ახლო აღმოსავლეთში. ერთი გუნდი — საიმედო პარტნიორი ყველა ეტაპზე.'],
        ...
        <div class="service-card__body">
          ...
          <div class="service-card__footer">
            ...
            <div class="service-card__footer-actions service-card__footer-actions--details-only">
              <button class="btn btn--small service-card__details-btn" type="button" data-open-service="${esc(item.id)}">${esc(t('buttons.details'))}</button>
            </div>
          </div>
        </div>
      </article>
  ...
  function bindGlobalEvents() {
    ...
    document.addEventListener('submit', (event) => {
      ...
      if (success) success.textContent = t('forms.success');
      showToast(t('forms.success'));
  ...
  function dispatchLead(lead) {
    if (window.DFG_BACKEND && window.DFG_BACKEND.isConfigured()) {
      window.DFG_BACKEND.saveLead(lead).catch((error) => console.warn('Lead backend save failed', error));
    }
    const integrations = data.settings?.integrations || {};
    const endpoint = String(integrations.leadWebhookUrl || '').trim();
    if (!integrations.leadWebhookEnabled || !endpoint) return;
    const payload = {
      source: 'DIANAFARM GROUP website',
      siteVersion: data.version || 'v123',
      lead
    };
    try {
