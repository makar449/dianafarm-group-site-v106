(function () {
  ...
  function bindEvents() {
    $('#loginForm')?.addEventListener('submit', (event) => {
      ...
      if (String(password) === String(data.settings.adminPassword || 'dianafarm2026')) {
    ...
    document.addEventListener('click', (event) => {
      ...
      if (saveAllButton) {
        event.preventDefault();
        withButtonFeedback(saveAllButton, () => {
          saveData();
          draftData = clone(data);
          saveDraftData(draftData);
          updateAdminStatus('Опубликовано', 'Live-версия обновлена. Если Supabase включён — данные уходят в общую базу.');
      ...
      if (publishDraft && confirm('Опубликовать текущий черновик на сайт?')) {
        ...
        saveData();
        renderActiveTab();
        toast('Черновик опубликован на сайт');
        return;
      }
      ...
      if (duplicate) {
        duplicateItem(duplicate.dataset.duplicate, Number(duplicate.dataset.index));
        return;
      }

      const del = event.target.closest('[data-delete]');
      if (del) {
        deleteItem(del.dataset.delete, Number(del.dataset.index));
        return;
      }

      const toggle = event.target.closest('[data-toggle-visible]');
      if (toggle) {
        toggleVisible(toggle.dataset.toggleVisible, Number(toggle.dataset.index));
        return;
      }
  ...
  async function syncSiteFromRemote(silent = false) {
    ...
    try {
      ...
      if (!remote) {
        ...
        if (!silent) toast('Создана первая запись сайта в Supabase');
        return;
      }
      data = mergeDefaults(clone(DEFAULT_DATA), remote);
      localStorage.setItem(DATA_KEY, JSON.stringify(data));
      renderActiveTab();
      if (!silent) toast('Данные сайта загружены из Supabase');
    } catch (error) {
      console.warn('Supabase site sync failed', error);
      if (!silent) toast('Supabase недоступен, работаем локально');
    }
  }
  ...
  async function pushSiteDataToRemote() {
    ...
    try {
      ...
      console.warn('Supabase site save failed', error);
      toast('Сохранено локально. Supabase не принял данные.');
    }
  }
...
function openEditor(collectionName, index) {
  ...
  function saveEditor(form, mode = 'publish') {
    ...
    updateAdminStatus('Опубликовано', 'Фото и текст доступны на сайте в этом браузере. При Supabase — в общей базе.');
    toast('Опубликовано на сайте. Фото и текст уже доступны.');
  }
  ...
  function duplicateItem(collectionName, index) {
  ...
  function deleteItem(collectionName, index) {
  ...
  function toggleVisible(collectionName, index) {
  ...
  function renderContactsEditor() {
    ...
    $('#adminContent').innerHTML = `
      <form class="admin-card admin-card-v228" data-contacts-form>
        ...
        <details class="admin-details-v228"><summary>Пароль админ-панели</summary><label class="field"><span>Пароль</span><input type="text" name="adminPassword" value="${esc(data.settings.adminPassword || '')}"><small class="form-note">Для боевого сайта нужен серверный вход. Это быстрый режим для статической версии.</small></label></details>
...
function contactGroupHTML(key, contact) {
  ...
  function applyContactsFormTo(targetData, form) {
    ...
    targetData.settings.adminPassword = formData.get('adminPassword') || targetData.settings.adminPassword;
    return targetData;
  }
    ...
    $('#adminContent').innerHTML = `
      <form class="admin-card admin-card-v228" data-integrations-form>
        ...
        <div class="admin-integration-status-v228 ${window.DFG_BACKEND?.isConfigured() ? 'is-on' : ''}">
          <strong>${window.DFG_BACKEND?.isConfigured() ? 'Supabase подключён' : 'Локальный режим'}</strong>
          <span>${window.DFG_BACKEND?.isConfigured() ? 'Изменения доступны в общей базе.' : 'Данные сохраняются только в этом браузере.'}</span>
        </div>
...
function leadsTable(leads) {
    ...
    return `<div class="admin-table-wrap admin-table-wrap-v228"><table class="admin-table admin-table-v228"><thead><tr><th>Дата</th><th>Клиент</th><th>Тип</th><th>Детали</th><th>Статус</th><th></th></tr></thead><tbody>${leads.map((lead) => `
      <tr>
        ...
        <td><select data-lead-status="${esc(lead.id)}"><option value="new" ${lead.status === 'new' ? 'selected' : ''}>Новая</option><option value="in-progress" ${lead.status === 'in-progress' ? 'selected' : ''}>В работе</option><option value="done" ${lead.status === 'done' ? 'selected' : ''}>Готово</option></select></td>
