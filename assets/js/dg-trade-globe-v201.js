...
(function(){
  'use strict';
  var VERSION = 'v201-proportional-safe-scale-trade-globe';
  var state = window.DG_TRADE_GLOBE_V201 = {
    version: VERSION,
    mounted: false,
    frames: 0,
    canvas: false,
  ...
  window.DG_TRADE_GLOBE_V195 = state; window.DG_TRADE_GLOBE_V198 = state; window.DG_TRADE_GLOBE_V200 = state;
  ...
  function computeStageBox(){
    ...
    var desired = Math.round((w >= 1680 ? 455 : (w >= 1440 ? 415 : (w >= 1180 ? 380 : (w <= 980 ? 325 : 350)))) * 1.3);
    var size = desired;
    ...
    if(layout && layout.hero){
      ...
      if(copy && proof){
        ...
        if(gapWidth > 210){
          var maxByGap = Math.max(245, gapWidth - 22);
      ...
      var availableH = Math.max(250, hero.bottom - top - 22);
  ...
  function applyResponsivePosition(root){
    ...
    forceStyle(root, {
      ...
      'pointer-events':'none', overflow:'visible', isolation:'isolate', contain:'none', 'transform-origin':'50% 50%', 'container-type':'inline-size'
    });
  ...
  function ensureRoot(){
    ...
    var routeLabels = root.querySelectorAll('.dg-v195-route-label');
    var box = computeStageBox();
    routeLabels.forEach(function(label){ label.style.fontSize = Math.max(13, Math.min(21, box.size * 0.05)) + 'px'; label.style.padding = Math.max(5, box.size * 0.016) + 'px ' + Math.max(10, box.size * 0.028) + 'px'; });
    return root;
  }
  function setupCanvas(root){
    ...
    function resize(){
      ...
      root.querySelectorAll('.dg-v195-route-label').forEach(function(label){ label.style.fontSize = Math.max(13, Math.min(21, W * 0.05)) + 'px'; label.style.padding = Math.max(5, W * 0.016) + 'px ' + Math.max(10, W * 0.028) + 'px'; });
    }
    resize();
