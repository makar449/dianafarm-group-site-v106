...
(function(){
  ...
  var state = window.DG_PRO_SCENES_V201 = {version:VERSION, mounted:false, frames:0, scene:null, kind:null, errors:[], canvas:false, rootExists:false, visible:false};
  window.DG_PRO_SCENES_V196 = state; window.DG_PRO_SCENES_V198 = state; window.DG_PRO_SCENES_V200 = state;
  ...
  function computeStageBox(){
    ...
    var desired = Math.round((w >= 1680 ? 470 : (w >= 1440 ? 425 : (w >= 1180 ? 390 : (w <= 980 ? 335 : 360)))) * 1.3);
    var size = desired;
    ...
    if(layout && layout.hero){
      ...
      var headerBottom = header ? header.getBoundingClientRect().bottom : 86;
      top = clamp(Math.round(hero.top + hero.height * 0.145), Math.round(headerBottom + 28), Math.round(h * 0.33));
      if(copy && proof){
        var gapLeft = copy.right + 26;
        var gapRight = proof.left - 26;
        var gapWidth = gapRight - gapLeft;
        if(gapWidth > 210){
          var maxByGap = Math.max(250, gapWidth - 22);
          size = Math.min(size, maxByGap);
      ...
      if(copy){
        if(left < copy.right + 14) left = Math.round(copy.right + 14);
      }
      if(proof){
        if(left + size > proof.left - 14) left = Math.round(proof.left - size - 14);
      }
      var availableH = Math.max(260, hero.bottom - top - 20);
      size = Math.min(size, availableH);
      if(proof && left + size > proof.left - 14) left = Math.round(proof.left - size - 14);
      if(copy && left < copy.right + 14) left = Math.round(copy.right + 14);
    }
  ...
  function applyResponsivePosition(root){
    ...
    forceStyle(root, {
      ...
      'pointer-events':'none', 'transform-origin':'50% 50%', 'container-type':'inline-size'
    });
  }
  ...
  function ensureRoot(kind){
    ...
    var label = root.querySelector('.dg-pro-v196-label');
    if(label){ var box = computeStageBox(); label.style.fontSize = Math.max(15, Math.min(28, box.size * 0.061)) + 'px'; label.style.letterSpacing = Math.max(1.4, box.size * 0.005) + 'px'; }
    return root;
  }
  ...
  function setupCanvas(root){
    ...
    function resize(){
      ...
      var label = root.querySelector('.dg-pro-v196-label');
      if(label){ label.style.fontSize = Math.max(15, Math.min(28, W * 0.061)) + 'px'; label.style.letterSpacing = Math.max(1.4, W * 0.005) + 'px'; }
    }
    resize();
    window.addEventListener('resize', resize, {passive:true});
    // v200: no scroll listener; scene is locked to its initial document position.
    state.documentBound = true; state.safeScale = true;
  ...
  ready(function(){
    try {
      ...
      console.error('[DIANAFARM v198 premium fixed scenes]', e);
    }
  });
})();
